...
public class Database extends SQLiteOpenHelper {
    ...
    public boolean inputdata(AddDatabase list) {
    ...
    public void readdata(ArrayList<AddDatabase> daftar){
        Cursor cursor = this.getReadableDatabase().rawQuery("select todo, description, priority from "+nama_tabel, null);
        while (cursor.moveToNext()){
            daftar.add(new AddDatabase(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
        }
    }
}
